...
def get_subjects(db: Session, user_id: UUID, skip: int = 0, limit: int = 100) -> List[Subject]:
    """
    Recupera as disciplinas do usuário.
    """
    return db.query(Subject).filter(Subject.user_id == user_id).offset(skip).limit(limit).all()

def get_subject_by_id(db: Session, subject_id: UUID, user_id: UUID) -> Optional[Subject]:
    """
...
def create_subject(db: Session, subject: SubjectCreate, user_id: UUID) -> Subject:
    """
...
def update_subject(db: Session, subject_id: UUID, subject_update: SubjectUpdate, user_id: UUID) -> Optional[Subject]:
    """
...
def get_subject_with_progress(db: Session, subject_id: UUID, user_id: UUID) -> Optional[SubjectWithProgress]:
    """
    ...
    completed_tasks = db.query(Task).filter(Task.subject_id == subject_id, Task.status == "COMPLETED").count()
