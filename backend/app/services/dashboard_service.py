...
def get_dashboard_metrics(db: Session, user_id: int) -> DashboardResponse:
    ...
    tasks_pending = user_tasks_query.filter(Task.status == "pendente").count()
    tasks_in_progress = user_tasks_query.filter(Task.status == "em_andamento").count()
    tasks_completed = user_tasks_query.filter(Task.status == "concluida").count()
    ...
    for subject in subjects:
        ...
        subj_completed = db.query(Task).filter(Task.subject_id == subject.id, Task.status == "concluida").count()
